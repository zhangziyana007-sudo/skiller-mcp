...
<p align="center">
  <strong>为 Claude Code 打造的一站式 AI Agent 技能管理平台</strong><br>
  本地仓库 · 3 种安装模式 · 社区共享 · 可视化 Dashboard · 项目级配置
</p>
...
```
本地仓库（统一视图）
├── ~/.claude/skills/                    ← 全局 Skill（Agent 按需加载）
└── ~/.claude/skiller/data/repository/   ← 社区下载的技能
```
...
技能统一存储在本地仓库中，安装到项目时支持 3 种模式：

| 模式 | 存储位置 | 触发方式 | Token 消耗 |
|------|----------|----------|-----------|
| **全局 Skill** | `~/.claude/skills/` | Agent 按需加载 | 省 |
| **常驻 Rule** | 项目 `CLAUDE.md` | 始终生效 | 多 |
| **按需 Rule** | `.claude/rules/` | Agent 根据描述按需加载 | 省 |
...
Skiller 通过环境变量 `SKILLER_PLATFORM` 支持多平台：
...
│   ├── config.ts          # 平台感知配置模块（路径/品牌名/安装模式）
...
| 版本 | 日期 | 主要变更 |
|------|------|---------|
| **v2.5.0** | 2026-04-10 | Claude Code 适配 + 平台感知配置 + 3 种安装模式 |
